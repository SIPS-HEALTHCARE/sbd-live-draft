...
const ENDOSCOPY_MODULES = [
 {id:'en-01',num:1,title:'Flexible Endoscope Reprocessing',subtitle:'High-Level Disinfection · AAMI ST91 / SGNA Standards',domain:'Endoscopy',
  desc:'The complete flexible endoscope reprocessing workflow: pre-cleaning at point of use through storage, documentation, and traceability. Assigned only to staff who work endoscopy.',
  sections:['1. Why Endoscope Reprocessing Matters','2. Understanding Endoscope Anatomy','3. Personal Protective Equipment','4. Pre-Cleaning At Point Of Use','5. Transport To Reprocessing','6. Leak Testing','7. Manual Cleaning','8. High-Level Disinfection','9. Rinsing','10. Drying','11. Storage','12. Documentation & Traceability','13. Troubleshooting','14. Quick Reference: Complete Workflow'],
  sectionContent:[
  ...
  // ── G1: Knowledge (14 items, auto-scored, ALL required to pass) ──────────
...
function assignEndoModule(staffId,moduleId,assignedBy,trigger){
 ...
 if(!p){
   ...
   // note 2026-08-28). Remove this seed and wire a real gate2 UI if a bank
   // arrives later — everything else (unlock, completion) already checks G1
   // alone, so this is the only line that changes.
   p={staffId,moduleId,g1:{status:'open',score:0,attempts:[]},g2:{status:'pass',score:100,attempts:[]},g3:{status:'open',items:[]},complete:false};
...
function saveEndoGateScore(staffId,moduleId,score,total){
 if(!DB.foundationsProgress) DB.foundationsProgress=[];
 let p=DB.foundationsProgress.find(x=>x.staffId===staffId&&x.moduleId===moduleId);
 if(!p){p={staffId,moduleId,g1:{status:'open',score:0,attempts:[]},g2:{status:'pass',score:100,attempts:[]},g3:{status:'open',items:[]},complete:false};DB.foundationsProgress.push(p);}
 const g=p.g1;
...
function markEndoG3Item(staffId,moduleId,itemId,confirmed,confirmedBy){
 ...
 if(confirmed&&!p.complete&&!endoObsReady(p)){
   if(typeof toast==='function') toast('Observation is locked: the Knowledge gate must be passed first (14 of 14 correct).','err');
   return;
 }
...
function renderSEndoscopy(){
 ...
 ENDOSCOPY_MODULES.forEach(m=>{
   ...
   html+='<div style="display:flex;gap:4px;align-items:center" title="Gate 1: Knowledge | Gate 3: Observation">';
   html+=fndGateBadge(gates.g1.status)+fndGateBadge(gates.g3.status)+'</div></div>';
   ...
   html+='<div class="fnd-gate-lbl">'+fndGateBadge(gates.g1.status)+'<span>Knowledge'+(gates.g1.status==='pass'?' (Passed)':'')+'</span></div>';
...
function renderEndoModuleTab(m,s,gates,tab){
 ...
 html+=tabBtn('gate3','Gate 2: Observation',tab==='gate3');
 html+='</div>';
 if(tab==='content'){
...
// every attempt sees the same bank (nothing to sample — 14 items total),
...
function renderEndoGateAssessment(m,s){
 ...
 h+='<div style="font-size:12px;color:#94a3b8;margin-bottom:16px">Answer every question. All 14 must be correct to pass. You may retake this as many times as needed.</div>';
 if(passed){
   ...
   h+='<div style="font-size:16px;font-weight:700;color:#4ade80">Gate Complete &mdash; 14 of 14 correct</div>';
   ...
   h+='<div style="background:rgba(196,154,32,.08);border:1px solid rgba(196,154,32,.25);border-radius:var(--r);padding:10px 14px;margin-bottom:16px;font-size:12px;color:#94a3b8">Last attempt: <b style="color:#fbbf24">'+g.score+'%</b>. All 14 items must be correct to pass &mdash; review and try again.</div>';
 }
...
function renderEndoG3View(m,s,gates){
 ...
 if(!gates.complete&&!endoObsReady(gates)){
   h+='<div style="background:rgba(148,163,184,.08);border:1px solid rgba(148,163,184,.25);border-radius:var(--r);padding:12px 14px;margin-bottom:16px;font-size:12px;color:#94a3b8">Observation unlocks after the <b style="color:#e2e8f0">Knowledge</b> gate is passed (14 of 14).</div>';
 }
...
// ── Leader: Training-table cell (mirrors scriptsCellHTML) ──────────────────
function endoscopyCellHTML(staffId){
 const a=getEndoAssignments(staffId)[0]||null;
 const canAssign=endoCanAssign();
 const isMaster=!!(ST.user&&ST.user.role==='master_admin');
 if(!a){
   return canAssign
     ? '<button class="btn btn-ghost btn-xs" onclick="hAssignEndoscopyModal(\''+staffId+'\')">Assign</button>'
     : '<span class="tc-muted">None</span>';
 }
 let h='<span class="'+(a.status==='completed'?'tc-ok':'tc-warn')+'" style="font-size:11.5px">'+(a.status==='completed'?'Completed':'Assigned')+'</span>';
 h+=' <button class="btn btn-ghost btn-xs" onclick="hEndoStaffDetail(\''+staffId+'\')">View</button>';
 if(isMaster) h+=' <button class="btn btn-ghost btn-xs" style="border-color:rgba(239,68,68,.4);color:#f87171" onclick="hUnassignEndo(\''+staffId+'\',\''+a.moduleId+'\')">Unassign</button>';
 return h;
}
...
function hAssignEndoscopyModal(staffId){
 ...
 let html='<div style="margin-bottom:12px;font-size:13px;color:var(--txt2)">Assign endoscopy training to <strong style="color:var(--txt)">'+fullName(s)+'</strong>. This is not part of the belt curriculum — nobody else at their belt sees it.</div>';
...
function hEndoStaffDetail(staffId){
 ...
 ENDOSCOPY_MODULES.forEach(m=>{
   ...
   html+='<div style="display:flex;gap:4px;align-items:center">'+fndGateBadge(gates.g1.status)+fndGateBadge(gates.g3.status);
   ...
   html+='<span>Knowledge: '+(gates.g1.status==='pass'?'<span class="tc-ok">'+gates.g1.score+'%</span>':'<span class="tc-muted">'+gates.g1.status+'</span>')+'</span>';
   html+='<span>Observation: '+(gates.g3.status==='pass'?'<span class="tc-ok">Confirmed</span>':'<span class="tc-warn">Pending</span>')+'</span>';
   html+='</div>';
   if(a){
...
function hUnassignEndo(staffId,moduleId){
 ...
 if(getEndoAssignments(staffId).length>0) hEndoStaffDetail(staffId); else if(typeof renderHTraining==='function') renderHTraining();
}
