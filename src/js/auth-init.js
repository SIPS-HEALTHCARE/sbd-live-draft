...
async function l3TriggerCheck(type, id, idx, forceNew){
  ...
  if(sec.text.length > 50){
    const sysPrompt = 'You are a sterile processing department (SPD) training assessment generator for the Sterile By Design (SBD) belt certification system. Generate a single multiple-choice comprehension question that tests understanding of the provided curriculum section. Respond ONLY with a JSON object, no markdown fences, no preamble. The JSON must have exactly these fields: q (the question string), opts (array of exactly 4 answer strings), correct (integer 0-3 for the correct answer index), explain (string explaining why the correct answer is right, 1-2 sentences). IMPORTANT: To prevent "test-wise" patterns, all 4 options must be approximately the same character length (within 15% variance). Do not make the correct answer significantly longer or more detailed than the distractors.';
    const userPrompt = 'Section Title: "'+sec.title+'"\n\nSection Content:\n'+sec.text+'\n\nGenerate one comprehension question about this specific content. Make it test real understanding, not just word recall. The wrong answers should be highly plausible to a professional and avoid "absolute" language like "always" or "never" unless it is factually part of a protocol. Ensure all options have a similar technical depth and length.';
...
async function initAppData(){
  ...
  try {
    ...
    window.DB.foundationsAssignments = (fndAssignments||[]).map(a=>({ id:a.id, staffId:a.staff_id, moduleId:a.module_id, assignedBy:a.assigned_by, type:a.assignment_type||a.type, trigger:(a.trigger_event!=null?a.trigger_event:a.trigger), facilityId:a.facility_id||null, assignedDate:a.assigned_date, status:a.status, mode:a.mode||'take' }));
    ...
    window.DB.instrumentAssignments = (instAssignments||[]).map(a=>({ id:a.id, staffId:a.staff_id, moduleId:a.module_id, assignedBy:a.assigned_by, type:a.assignment_type||a.type, trigger:(a.trigger_event!=null?a.trigger_event:a.trigger), facilityId:a.facility_id||null, assignedDate:a.assigned_date, status:a.status, mode:a.mode||'take' }));
