...
async function initAppData(){
  try {
    const [facs, staff, systems, users] = await Promise.race([
      Promise.all([
      ...
      new Promise((_,rej)=>setTimeout(()=>rej(new Error('Initial data load timeout')), 15000))
    ]);

    // Hydrate Global DB
    DB.facilities = (facs||[]).map(mapFacilityFromBackend);
    DB.staff = (staff||[]).map(mapStaffFromBackend);
    DB.hospitalSystems = (systems||[]).map(mapHospitalSystemFromBackend);
    DB.users = (users||[]).map(mapUserFromBackend);

    console.log(`SBD Platform: Loaded ${DB.facilities.length} facilities, ${DB.staff.length} staff, ${DB.users.length} users.`);
    ...
    console.error('SBD Platform: Failed to load initial data:', e.message);
  }
}
