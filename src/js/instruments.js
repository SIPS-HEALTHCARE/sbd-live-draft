...
function hDoAssignInst(sid){if(ST.user&&(ST.user.role==='staff_admin'||ST.user.role==='assessor')){toast('Assessors cannot assign modules','err');return;}const cbs=document.querySelectorAll('.inst-assign-cb:checked');if(!cbs.length){toast('Select at least one','err');return;}const nm=ST.user?ST.user.name:'Manager';cbs.forEach(cb=>assignInstModule(sid,cb.value,nm,'remediation',null));closeModal();toast(cbs.length+' module'+(cbs.length>1?'s':'')+' assigned','ok');renderHInstruments();}
function hAssignAllInst(sid){if(ST.user&&(ST.user.role==='staff_admin'||ST.user.role==='assessor')){toast('Assessors cannot assign modules','err');return;}assignAllInstModules(sid,ST.user?ST.user.name:'Manager');toast('All 4 instrument modules assigned','ok');renderHInstruments();}
