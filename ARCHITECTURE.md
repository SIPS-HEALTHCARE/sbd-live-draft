...
| `sbd_practice_attempts` | P0.3 append-only log of self-graded practice runs (one row per completed Study & Practice set, written by `savePracticeScore()` in ui-views.js via `SB.logPracticeAttempt`). Read only by David's per-staff serializers; no DB rollup reads it. `leadership` (#1119, nullable) = staffer held Blue/Brown/Black at attempt time, via `isLeadershipBelt()` in logic.js; null = pre-tag row. | `staff_id` → `staff`, `fid` → `facilities` |
...
| `sbd-admin-sessions` | **#21:** admin in-progress assessment tracker. Service-role read of `sbd_assessment_sessions`, `active` + unexpired only, scoped to the caller's facilities. `master_admin` / `staff_admin` / `system_admin`, T33 MFA-guarded. | `renderInProgressPanel()` (ui-views.js) |
| `sbd-leader-sessions` | **#1180 (board 151):** read-only leader view of the same table — `facility_admin` / `hospital`, own facility only, **every** status (all prod rows are completed/expired). Staff name, module, status, started/completed. No PIN values, no writes. `sbd-admin-sessions` and its three roles deliberately unchanged. | `renderLeaderSessionsPanel()` (ui-views.js) |
...
| `sbd-emails` / `sbd-send-emails` | Email notifications (template `mfa_email_code` added by #1144). `sbd-emails` has two door types: named payloads from the frontend (`placement_started`, `placement_completed`, `registration_denied`) and a DB-trigger webhook branch behind `Authorization: Bearer <WEBHOOK_SECRET>`, fail-closed. **#1225 (2026-09-12):** the only trigger that uses that branch is `on_auth_user_created` on `auth.users` → `public.handle_welcome_email()`, and it now reads the bearer from **Vault** secret `sbd_emails_webhook_secret` instead of inlining it; rotating means `vault.update_secret` **and** `supabase secrets set WEBHOOK_SECRET`, in that order, then redeploy. The trigger fails **open** (warning, signup proceeds) because it runs inside the `auth.users` insert. ⚠️ `sbd-emails` has **no branch for `INSERT` on `auth.users`**, so that post returns `200 {ignored:true}` and **no welcome email exists** — the name is aspirational; see TASKS T131. | Frontend (`notifyPlacementEvent`, `ui-views.js:16932`) + `on_auth_user_created` |
...
- **#1121 observation carries no score (board 148, ledger T132):** G3 is a leader-confirmed
  checklist, so its pass writes `status` alone — the four `markG3Item` twins
  (foundations/instruments/endoscopy/preceptor) no longer stamp `score:100` on the last tick
  or `score:0` on a revoke. `fiModuleSummary`/`prcModuleSummary` report
  `gates.observation.score` as **null** when absent instead of a stand-in `0`; nothing renders
  it (every surface reads `.status`). The `na:true` shapes (#720 g2, #1123 read mode, #1143)
  still carry `score:100` — those are written and pinned by `sbd_fi_progress_guard`
  server-side, so changing them is a migration. Verify: `node scripts/verify-1121-observation-score.js`.
...
- **#1123 read vs take (`20260908150000`, board 137):** `mode` on both assignment rows, `'take'` (default, today's 3 gates) or `'read'`. Chosen once per assign batch in `hAssignFndModal`/`hAssignInstModal` (`fiModeSelectHTML`). Read mode: staff reader hides the gate tabs and shows the `FI_READ_ONLY_LABEL`; `confirmFndRead`/`confirmInstRead` write the confirmation on **g1** (`{status:'pass',read:true,date}`), and `sbd_fi_progress_guard` pins g2/g3 to a not-applicable pass for a read assignment (same mechanism as #720's en-% g2), so `complete` = g1 pass. The guard now also **mirrors `complete` onto `assignment.status`** — staff cannot PATCH the assignment row (leader-only policy), so this is the only way a staffer-earned completion reaches the status every counter reads. Helpers live in foundations.js (`fiMode`, `fiMarkRead`, `fiReadConfirmHTML`, `fiReadStatusHTML`); instruments.js reuses them. Endoscopy/Scripts rows on `foundations_assignments` default to `take`. Verify: `node scripts/verify-1123-read-mode.js`, `supabase/verify/1123_assignment_mode_check.sql`. Decision: `docs/decisions/2026-09-08-1123-read-vs-take.md`.
...
- **Display name is "SBD Language" (#1181 tab, #1223 the rest).** Every user-visible label,
  tour step, card title and learner-guide section heading reads *SBD Language*; the 70
...
- **File:** `src/js/scripts-module.js`. `scriptSectionsForBelt(belt)` is the single definition
  ...
  language|approved language|forbidden language/i`); the Study & Practice SBD Language tab
  calls it too, so there is one definition, not two. ⚠️ **This regex is load-bearing against
  the curriculum titles** — #1223 retitled all ten sections to "SBD LANGUAGE", so dropping
